...
## Trade-offs / Qué haría con más tiempo

- **Sin tests automatizados**: no hay proyecto xUnit para el backend ni tests de integración/E2E versionados para el frontend (sí verifiqué manualmente cada feature con Playwright durante el desarrollo, pero esos scripts no quedaron en el repo). Con más tiempo agregaría un proyecto de tests para los `Services` del backend y un puñado de tests E2E críticos (login, dejar reseña, editar/eliminar) corriendo en el CI.
- **Restablecer contraseña sin email real**: el link de reset se muestra en pantalla en vez de enviarse por correo (ver Decisiones). Con más tiempo integraría un servicio transaccional gratuito (Resend o Brevo) con dominio verificado.
...
## Checklist de requisitos del brief

**Obligatorios**

- [x] Página de inicio con listado, búsqueda por título/autor/categoría
- [x] Detalle de libro (título, autor, categoría, resumen)
- [x] Dejar reseñas (calificación 1-5 + comentario)
- [x] Ver reseñas de otros usuarios, ordenadas por más recientes
- [x] Autenticación: registro, login, logout; solo usuarios autenticados dejan reseñas
- [x] Responsive y accesible
- [x] Desplegada en una plataforma de alojamiento con pipeline de CI/CD

**Opcionales**

- [x] Filtrar libros por categoría
- [x] Ver reseñas propias y editarlas/eliminarlas
- [x] Perfil propio (usuario, correo, foto — foto vía URL, ver Trade-offs)
- [x] Restablecer contraseña (modo desarrollo, ver Trade-offs)
